# kiosk-ws-broadcast Specification

## Purpose

Defines the kiosk runtime's WebSocket broadcast layer: how `Bun.serve()` is extended with a `websocket` handler in kiosk mode, the on-the-wire `RaceUpdate` vocabulary shared between backend and frontend, the curated replay sent to each newly-connected client, and the bridge that fans out domain bus events to all subscribed sockets. This capability owns the contract between the in-process `TypedEventBus<RaceEventMap>` and the network.
