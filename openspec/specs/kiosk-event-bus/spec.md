# kiosk-event-bus Specification

## Purpose

Defines the kiosk runtime's in-process event bus and lap-detection pipeline. Provides a generic, type-safe wrapper over Node's `EventEmitter` (`TypedEventBus<M>`), a one-shot loader for the track centerline GPX, and a lap detector that consumes decoded samples, persists completed laps via Drizzle, and emits a typed `lap` event. This capability owns the contract between telemetry ingest and downstream consumers (future WS broadcast, dashboard, alerts).
