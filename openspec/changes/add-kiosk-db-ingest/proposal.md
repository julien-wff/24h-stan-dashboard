...
- **Schema is pushed on startup** by shelling out to `drizzle-kit push` programmatically from the kiosk boot path, against the configured `KIOSK_DB_PATH`. The kiosk does not start serving until the push completes successfully; a failed push aborts startup with a non-zero exit. `drizzle.config.ts` is added at the repo root pointing `drizzle-kit` at `src/backend/kiosk/db/schema.ts`.
...
- **Add an ingest loop** in `src/backend/kiosk/ingest.ts` that consumes from the configured telemetry source, validates each packet against the JSON-contract shape, and writes one `raw_packets` row + one `decoded_samples` row per valid packet inside a single transaction. Invalid packets are logged and counted but do not crash the loop.
- **Wire the kiosk boot path** into `src/backend/index.ts`: when `APP_MODE=kiosk`, the entrypoint runs the schema push, opens the Drizzle connection, starts the ingest loop against the configured source, and only then starts `Bun.serve()`. Existing `/*` and `/api/hello*` routes are preserved unchanged. When `APP_MODE` is unset or `server`, none of the kiosk-mode wiring runs.
...
- **Files added**:
  ...
  - `src/backend/kiosk/db/push.ts` — startup wrapper that invokes `drizzle-kit push` against `KIOSK_DB_PATH`.
...
- **Files modified**:
  ...
  - `.gitignore` — add `data/`.
- **Dependencies added**: `drizzle-orm`, `drizzle-kit`.
- **Runtime behavior change**:
  - With `APP_MODE` unset (today's default), no new behavior — the server still serves the same React shell.
  - With `APP_MODE=kiosk` and `KIOSK_TELEMETRY_SOURCE=simulated`, the backend creates/migrates `./data/kiosk.db`, starts streaming simulated packets at ~1 Hz, and writes two rows per packet. The HTTP layer still serves the existing routes.
